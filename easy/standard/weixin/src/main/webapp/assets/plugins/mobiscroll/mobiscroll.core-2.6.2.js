...
//            },
        ...
        function read() {
            ...
            if(s.preset=="select"&&s.multiple==false){
            	that.temp = s.parseValue(elm.val() || '', that);
            }
            setVal();
        }
        ...
        */
        ...
        that.tap = function (el, handler) {
            ...
            /**
             * 绑定确定，取消的click事件，使click效果与tap一致
             * (浏览器时取消下列注释)
             */
           /* if(GLOBAL_NATIVE_FLAG == "OFF"){
        ...
        that.show = function (prevAnim) {
            ...
            $.each(s.wheels, function (i, wg) { // Wheel groups
                ...
                $.each(wg, function (j, w) { // Wheels
                    ...
                    if('date'==s.preset||'month'==s.preset){
                    	html += '</div></div><div class="dwwo"></div></div><div class="dwwoll"></div></div></td>';
                    }else if('select'==s.preset){
                    	html += '</div></div><div class="dwwo"></div></div><div class="dwwol"></div></div></td>';
                    }else if('year'==s.preset){
                    	html += '</div></div><div class="dwwo"></div></div><div class="dwwoYear"></div></div></td>';
                    }else if('list'==s.preset){
                    	html += '</div></div><div class="dwwo"></div></div><div class="dwwoll"></div></div></td>';
                    }else if('datetime'==s.preset){
                    	html += '</div></div><div class="dwwo"></div></div><div class="dwwoll"></div></div></td>';
                    }else{
                ...
                if('date'==s.preset){
                	html += '<div id="date_div" ></div>';
                }else if('month'==s.preset){
                	html += '<div id="month_div" ></div>';
                }else if('datetime'==s.preset){
                	if(i == 0){
                		html += '<div class="datetime_ymd"><span>年</span><span>月</span><span>日</span></div><div id="ymd_div" ></div>';
                	}else{
                		html += '<div class="datetime_hi"><span>时</span><span>分</span></div><div id="hi_div" ></div>';
                	}
                }else{
            ...
            if('list'==s.preset){
           	 html += '<div id="list_div_left" ></div>';
           	 html += '<div id="list_div_right" ></div>';
            }
            ...
            //为了正常显示下拉列表的红色框添加
            if('select'==s.preset){
            	var inputId=s.inputID;
            	//对返回或修改等情况  多选框默认打上勾
            	if(s.multiple==true&&$("#"+inputId).val()!=""&&$("#"+inputId).val()){
            		var selectText = ($("#"+inputId).val().replace(/\s+/g,"")).split(",");
            		for(var i=0;i <$(".dw-i").length; i++){
            			var iText = $($(".dw-i")[i]).text();
            			if(selectText.indexOf(iText) != -1){
            				$($(".dw-i")[i]).parent().addClass("dw-msel");
            				$($(".dw-i")[i]).parent().attr("aria-selected",true);
            			}
            		}
            	}
            	var $dwwol=$(".dwwol");
            	$dwwol.css("width",$(".dwwr").width()- 6);
            	$dwwol.css("margin-left",-(4+($(".dwwr").width()-$(".dwhl").width())/2)+"px");
             }
            if('year'==s.preset){
            	var $dwwoYear=$(".dwwoYear");
            	$dwwoYear.css("width",$(".dwwr").width()- 6);
            	$dwwoYear.css("margin-left",-(4+($(".dwwr").width()-$(".dwhl").width())/2)+"px");
             }
...
//                .on('keyup', onKeyUp);
        ...
        */
        ...
        that.select = function (callback) {
        	setTimeout(function(){
	            if (that.hide(false, 'set') !== false) {
	                setVal(true, 0, true);
	                event('onSelect', [that.val]);
	                if(callback!=undefined&&typeof callback == 'function'){
	                	s.callback.call(this,elm);
	                }
	            }
        	},350); 
        };
