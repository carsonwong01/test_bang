(function (factory) {
  ...
  FormCache.prototype = {
    ...
    init: function () {
      ...
      if (!$.isArray(defaults.attachments)) {
        defaults.attachments = [];
      }
      
      if (!$.isArray(defaults.richTexts)) {
        defaults.richTexts = [];
      }
      
      if (!$.isArray(defaults.returnLis)) {
        defaults.returnLis = [];
      }
    ...
    addListeners: function (flag) {
      var $controlsTemp;
      if (flag) {
        $controlsTemp = this.$form.find(this.defaults.controls.join()).not(':file').not(this.$controls);
        this.$controls = this.$form.find(this.defaults.controls.join()).not(':file'); //重新设置缓存的元素，保证上传控件添加的input元素被缓存
      }
      ...
      if (flag) {
        $controlsTemp.each(function(){
            $(this).trigger(EVENT_CHANGE);
        });
        //处理图片附件全清空情况
        for (var i=0; i < this.defaults.attachments.length; i++) {
          var name = this.defaults.attachments[i];
          if (!$("[name="+name+"]") || $("[name="+name+"]").length == 0) {
            this.update(name, "");
          }
        };
        //回报清空情况
        for (var i=0; i < this.defaults.returnLis.length; i++) {
          var name = this.defaults.returnLis[i];
          if (!$("[data-name="+name+"]") || $("[data-name="+name+"]").length == 0) {
            this.update(name, "");
          }
        };
      }
    ...
    change: function (e) {
	  ...
	  if($this.attr("data-name") && this.defaults.returnLis.join(",").indexOf($this.attr("data-name")) > -1){
      ...
      if (this.defaults.returnLis.join(",").indexOf(tmpName) > -1) {
          this.$controls.filter('[data-name*="' + tmpName + '"]').each(function () {
          	if(_this.defaults.returnLis.join(",").indexOf(tmpName) > -1){
          ...
          this.$controls.filter('[name*="' + tmpName + '"]').each(function () {
            if (isCheckboxOrRadio(input)) {
              ...
              if (_this.defaults.attachments.join(",").indexOf(tmpName) > -1) {
                val = JSON.stringify($(this).data());
              }else{
                val = $(this).val();
              }
    ...
    serialize: function () {
      ...
      this.$controls.each(function () {
        ...
        if (isCheckboxOrRadio(this)) {
          ...
          if (_this.defaults.attachments.join(",").indexOf(name) > -1) {
            val = JSON.stringify($(this).data());
          }else if(_this.defaults.returnLis.join(",").indexOf(name) > -1){
          	val = JSON.stringify($(this).data());
          }else{
            val = $(this).val();
          }
    ...
    getControllsCache: function (index) {
      var data = {};
      var cache = this.getCache(index);
      ...
      if ($.isPlainObject(cache)) {
        ...
        //默认图片id
        var coverImageId = cache["coverImageId"];

        var attachments = this.defaults.attachments;
        for (var i=0; i < attachments.length; i++) {
            var name = attachments[i];
          if (!name) {
            return;
          }
          ...
          if(value){
              ...
              for (var j=0; j < value.length; j++) {
                var obj = JSON.parse(value[j]);
                fileTemp = {};
                fileTemp.imageId = obj.batchNumber;
                fileTemp.imageUrl = obj.url;
                fileTemp.imageExt = obj.fileExt;
                fileTemp.imageName = obj.oldName;
                fileTemp.imageSize = obj.size;
                if (coverImageId && coverImageId == obj.batchNumber) {
                    fileTemp.isDefault = "1";
                }
                files.push(fileTemp);
              }
            data[name] = files;
        ...
        var richTexts = this.defaults.richTexts;
        for (var i=0; i < richTexts.length; i++) {
            var name = richTexts[i];
          if (!name) {
            return;
          }
          var value = cache[name];
        ...
        var returnLis = this.defaults.returnLis;
        for (var i=0; i < returnLis.length; i++) {
            var name = returnLis[i];
          if (!name) {
            return;
          }
          var value = cache[name];
  ...
  FormCache.DEFAULTS = {
    ...
    attachments: [ //图片附件input名称
        'imagesIds',
        'receiveCardImageIds',
        'recipientCardImageIds',
        'weddingPictureIds',
        'proveImgIds',
        'accountBookImgIds1',
        'accountBookImgIds2',
        ...
        'useProveImgIds',
        'projectProveImgIds',
        'organizationAptitudeIds'
    ],
    richTexts: [ //富文本、下拉框等特殊标签名称
        'projectDetails',
        'hospitalRegionId',
        'projectLabel'
    ],
