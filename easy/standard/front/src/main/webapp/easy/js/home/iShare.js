(function (root, factory) {
	...
	function iShare(options) {
		var defaults = {
					...
					image       : Util.getimg(),
					sites       : ['iShare_wechat','iShare_weibo','iShare_qzone','iShare_qq'],
					initialized : true,
					isTitle     : true,
					isAbroad    : false,
					WXoptions   : {
	...
	iShare.prototype = (function(){
		...
		var _templates = {
	        iShare_wechat			 : '',
	        iShare_weibo       : 'http://service.weibo.com/share/share.php?url={{URL}}&title={{TITLE}}%0a{{DESCRIPTION}}&pic={{IMAGE}}',
      	...
      	_icons = {
          iShare_wechat      : '<a class="other-login-icon other-login-weixin">&#xe613;</a>',
          iShare_weibo       : '<a class="centerIcon1 other-login-icon other-login-weibo">&#xe609;</a>',
          iShare_qzone       : '<a class="centerIcon2 other-login-icon other-login-qqZoom">&#xe610;</a>',
          iShare_qq          : '<a class="other-login-icon other-login-qqicon">&#xe60f;</a>'
      };
    /**
     ...
     */
			...
			for(var i = 0, item; item = _children[i++];){
				if(item.nodeType === 1){
					...
					if((item.className).indexOf('iShare_wechat') > -1){
						this.wx = new WX(item, this.settings.WXoptions);
					} else {
		...
		function _createShareElements(url, item, isWechat){
			...
			if(isWechat){
				this.wx = new WX(_e, this.settings.WXoptions);
		...
		return{
			...
			init: function() {
				...
				if(this.wx){
					...
					this.wx.init();
					// 加载qrcode库
					Util.loadjs(basePath + 'easy/js/home/qrcode.min.js', this.wx.startQR());
				}
			},
		bindEvent: function(){
			var me = this;
