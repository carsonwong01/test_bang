...
DM.Page.ready({
	"初始化" : function() {
        //初始化form表单缓存
        var cacheData = projectStartController.initFormCache($("#projectForm"), "otherRelief");
        
		projectStartController.readyProject();
		//初始化input实时显示填写字数
		projectStartController.initCalculate($("input[name=projectName],input[name=projectIntro]"));
		//初始化富文本控件
		var projectDetails = projectStartController.initKindEditor("projectDetails","建议详细描述救助的情况、资金用途。","projectDetailsTip");
