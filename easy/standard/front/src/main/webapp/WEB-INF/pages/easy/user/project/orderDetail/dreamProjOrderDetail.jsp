		...
		<div class="personal-con clearfix">
			...
			<div class="public-form_info mt20 public-minheight OrderBox">
				<ul class="cell">
					<!-- 只有从订单支持进来时才显示支持者昵称 -->
					<c:if test="${from=='ddzc' }">
						<li><div class="til">支持者：</div><div class="info">${orderDetail.nickName}</div></li>
					</c:if>
					...
					<li><div class="til">帮助梦想：</div><div class="info">${orderDetail.purposeText}</div></li>
					<li
						<c:if
					test="${orderDetail.status=='7' || orderDetail.status=='8' }"> class="divider-line pb20"</c:if>><div class="til">留言：</div><div
							class="info">${orderDetail.message}</div></li>
					<!-- 退款信息 -->
