...
<script id="projectOrderListTemplate" type="text/x-jquery-tmpl">
	...
	{{each(i,data) list}}
		 <tr>
			<td>{{= i+1}}</td>
			<td>{{= orderNo}}</td>
			<td title="{{= nickNameTips}}">{{= nickNameTips}}</td>
			<td>{{= supportAmount}}</td>
...
{{if status == '8'}}已退款{{/if}}
{{if status == '9'}}已失败{{/if}}
			</td><td><a  href="javascript:void(0)" onclick="projectManageController.goOrderDetail('{{= orderId}}','ckxq')" class="highlight">查看</a></td>
		</tr>
	{{/each}}
</script>
