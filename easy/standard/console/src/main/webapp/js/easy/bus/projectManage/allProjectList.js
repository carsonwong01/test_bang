...
var AllProjectControler = DM.constructor.sub({
	...
	getAllProjectListAjaxList : function() {
		...
		DM.ajax({
			url : "bus/projectManage/getProjectListAjax.do",
	...
	setAllProjectAjaxQuery : function(data) {
		...
		$("#projectData").empty();
		// 填充数据
		$('#allProjectListTemplate').tmpl(data.pageResult).appendTo(
				"#projectData");
		// 初始化分页标签
		DM.PageTags.init({
			...
			showPages : 10, // 显示记录数
			url : "bus/projectManage/getProjectListAjax.do", // 请求路径
	...
	"导出列表" : function() {
		...
		$("#export").bind("click", function() {
			DM.exportExcel({
				...
				"formId" : "dataForm",
				"fileName" : "所有项目列表.xls",
				"url" : "bus/projectManage/projectListExport.do"
			});
		});
	}
