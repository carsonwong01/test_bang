 ...
 *已删除项目列表js
 */
var PreheatingListControler = DM.constructor.sub({
	// 查询的Ajax
	getPreheatingListAjax : function() {
		var _self = this;
		/* myfn.AjaxFn("user/perInformation.do",$(".viewFramework-content"),$("#dataForm").serialize()); */
		DM.ajax({
			url : "bus/projectManage/preheatingListAjax.do",
			data : $("#dataForm").serialize(),
			success : function(data) {
				if (data) {
					_self.setPreheatingListAjaxQuery(data.data);
				}
			},
			error : function(data) {
	...
	// 项目列表填充
	setPreheatingListAjaxQuery : function(data) {
		var _self = this;
		$("#projectData").empty();
		// 填充数据
		...
		DM.PageTags.init({
			...
			showPages : 10, // 显示记录数
			url : "bus/projectManage/preheatingListAjax.do", // 请求路径
...
var controler = new PreheatingListControler();
// 页面加载时调用
DM.Page.ready({

	"项目管理" : function() {
		controler.getPreheatingListAjax();
	},
	...
	"导出列表" : function() {
		...
		$("#export").bind("click", function() {
			DM.exportExcel({
				"tableId" : "exportTable",
				"formId" : "dataForm",
				"fileName" : "已删除项目列表.xls",
				"url" : "bus/projectManage/preheatingListExport.do"
			});
		});
	}
