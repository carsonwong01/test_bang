 ...
 * 
 */
var ReturnProOrderListController = DM.Controller.sub({
			init : function() {
			},
			/*
			 * 支付记录列表
			...
			pageCallBack : function(data) {
				// 清空表格数据
				$("#returnProOrderList").empty();
				
				// 填充数据
				$('#returnProOrderListTemp').tmpl(data).appendTo("#returnProOrderList");
				$("#orderTotalCount").html(data.pageResult.recordCount);
				// 初始化分页标签
				DM.PageTags.init({
					"divId" : "returnProOrderListTag",
					"formId" : "dataForm",
...
//实例化控制器
var controller = new ReturnProOrderListController();
// 页面加载时调用
DM.Page.ready({
	"产品急销订单列表" : function() {
		//绑定查询事件
