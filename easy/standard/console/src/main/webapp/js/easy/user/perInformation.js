...
var PerInformationControler=DM.constructor.sub({
	...
	getPerInformationList:function(){
		  ...
		  /* myfn.AjaxFn("user/perInformation.do",$(".viewFramework-content"),$("#dataForm").serialize());*/
		   DM.ajax({
				url:"userManage/perInformationAjax.do",
	   ...
	   setperInformationAjaxQuery:function(data){
		      var _self=this;
	          $("#perInformationId").empty();
			  //填充数据
			  $('#perInformationTemplate').tmpl(data.pageResult).appendTo("#perInformationId");
			  //初始化分页标签
			  DM.PageTags.init({
			        ...
			        showPages:10,  //显示记录数
			        url:basePath+"userManage/perInformationAjax.do",  //请求路径
